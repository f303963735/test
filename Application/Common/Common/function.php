 ...
 * date 2016年7月13日下午2:40:58
 */
function sd($time=0, $type='d', $unit=1){
    ...
    switch ($type){
        case 'd':
            ...
            $end = mktime(0, 0, 0, date('m',$time), date('d',$time)+$unit, date('Y',$time)) - 1;
            break;
        case 'm':
            $start = mktime(0, 0, 0, date('m',$time), date('d',$time), date('Y',$time));
            $end = mktime(0, 0, 0, date('m',$time) + $unit, date('d',$time), date('Y',$time)) - 1;
            break;
        case 'fd':
            $start = mktime(0, 0, 0, date('m',$time), date('d',$time) - $unit, date('Y',$time));
            $end = $time;
            break;
    }
 ...
 * 每天的时间段
 * @param number $time
 * @param int $period 默认2小时分隔
 * author panfeng
 * email 89688563@qq.com
 * date 2016年7月13日下午3:16:23
 */
function period($time=0,$period=0,$type='d',$unit=1){
    
    $arr = sd($time, $type, $unit);
    $start = $arr['start'];
    $end = $arr['end'];
    
    $period = $period ? $period : 2*3600;
